...
def load_tfr(subject, batch, condition):
    f_main = os.path.join('data_tfr', 'batch_{}'.format(batch), 'sub-{}'.format(subject), 'ses-1')
...
if False: # plot example of tfr data
    data, chans, frex, times = load_tfr(get_subjects(batch=1)[0], 1, 'T')
...
def test_try_ipca_one_dataset():
    batch = 1
    subject = get_subjects(batch)[0]
    condition = 'T'
    data, chans, frex, times = load_tfr(subject, batch, condition)
    fi = 0
    tp = data[:, :, fi, :].copy()
    tp2 = transpose_X(tp)
    if False:
...
def ipca_frequency(freq):
    ...
    conditions = ['T', 'R', 'N']
    _, chans, frex, times = load_tfr(subjects[0], batch, conditions[0])
...
if False: # get pc fit per frequency
    batch = 1
    _, _, frex, _ = load_tfr(get_subjects(batch)[0], batch, 'T')
    for fi, f in enumerate(frex):
        ipca_frequency(f)


def export_pc(subject, batch):
    ...
    for condi, c in enumerate(conditions):
        ...
        data, _, frex, _ = load_tfr(subject, batch, c)
        print('Done.')
...
if False:
    batch = 2
    subjects = get_subjects(batch)[40:]
    for si, s in enumerate(subjects):
...
def export_selected_features():
    df = pd.read_csv(os.path.join('results_idv_decoder', 'batch_{}'.format(batch), 'feature_idx.csv'))
    fidx = df.fid.tolist()
    _, _, frex_raw, _ = load_tfr(get_subjects(batch=1)[1], 1, 'T')
    frex = []
    pc = []
    ...
    df = pd.DataFrame({'fid': fidx,
                       'frequency': frex,
                       'pci': pc})
    df.to_csv(os.path.join('results_idv_decoder', 'batch_{}'.format(batch), 'features.csv'), index=False)
    return None
...
def plot_feature_loadings():
    feats = pd.read_csv(os.path.join('results_idv_decoder', 'batch_{}'.format(batch), 'features.csv'))
