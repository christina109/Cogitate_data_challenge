...
def load_data(subject, batch, condition):
    f_main = os.path.join('data_pc', 'batch_{}'.format(batch), 'sub-{}'.format(subject), 'ses-1')
    with open(os.path.join(f_main, 'pc_{}.npy'.format(condition)), 'rb') as f:
        data = np.load(f)
    return data


if False: # plot example of input
    X = load_data('CA103', 1, 'T')
    f = plt.figure()
    plt.pcolormesh(X[10,:,:])



def load_dataset(subject, batch):
    conditions = get_condition_code()
    for ci, c in enumerate(['T', 'R', 'N']):
        tp = load_data(subject, batch, c)
        #print(tp.shape[0])
        if ci == 0:
            X = tp.copy()
...
def feature_testing_run(batch, run, use_features=None):
    subjects = get_subjects(batch)
    for si, s in enumerate(subjects):
        print('Cross-validation for subject ({}/{})...'.format(si+1, len(subjects)))
        t0 = time.time()
        f_save_main = os.path.join('results_idv_decoder', 'batch_{}'.format(batch), 'sub-{}'.format(s), 'ses-1',
                                   'run-{}'.format(run))
        check_folder(f_save_main)
        X, y = load_dataset(s, batch)
...
def cv_session():
    batch = 1  # feature selection using the training sample
    run = 0
    n_feat = [114]
    feature_testing_run(batch=batch, run=run, use_features = None)
    feats = eval_features(batch, run)
    n_feat.append(len(feats))

    while n_feat[-1] < n_feat[-2] and n_feat[-1] > 0:
        run += 1
        feature_testing_run(batch=batch, run=run, use_features=feats)
        feats = eval_features(batch=batch, run=run)
        n_feat.append(len(feats))
    return n_feat


if False: # train final models (for both datasets)
    batch = 2 # change
    use_features = eval_features(batch=1, run=3)  # use features selected in the training dataset (batch 1)
    subjects = get_subjects(batch)
    for si, s in enumerate(subjects):
...
def predict_idv_decoder(subject, batch, cvi):
    use_features = eval_features(batch=1, run=3) # use features selected in the training dataset (batch 1)
    if not cvi == 99:
        model = load_model(os.path.join('results_idv_decoder', 'batch_{}'.format(batch), 'sub-{}'.format(subject),
                                        'ses-1', 'run-4', 'model_lnpo{}.h5'.format(cvi)))
    else:
        model = load_model(os.path.join('results_idv_decoder', 'batch_{}'.format(batch), 'sub-{}'.format(subject),
                                        'ses-1', 'model_lnpo{}.h5'.format(cvi)))
    X, y = load_dataset(subject, batch)
    X = scaling_X(X)
    X = X[:, use_features,:]
    X = np.expand_dims(X, axis=-1)
...
def summarize_performance_idv_decoder(batch):
    subjects = get_subjects(batch)
    acc = []
    trp = {0:[], 1:[], 2:[]}
    print('Getting performance...')
    for si, s in enumerate(subjects):
        y, y_hat = predict_idv_decoder(s, batch, 99)
    ...
    df.to_csv(os.path.join('results_idv_decoder', 'batch_{}'.format(batch), 'summary_performance_final.csv'), index=False)
    print(df.sort_values(by='bacc', ascending=False).round(3))
    return None


def export_feat_idx():
    feats = eval_features(batch=1, run=3)
    feats = pd.DataFrame({'fid': feats})
    feats.to_csv(os.path.join('results_idv_decoder', 'batch_{}'.format(batch), 'feature_idx.csv'), index=False)
    return None
