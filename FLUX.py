...
# path management
batch = 2  # change
bids_root  = os.path.join('raw', 'batch_{}'.format(batch), 'meg')
deriv_root = os.path.join('derivatives', 'batch_{}'.format(batch))
meg_suffix = 'meg'
max_suffix = 'raw_sss'
ann_suffix = 'ann'
ica_suffix = 'ica'
...
# test only
#subject = 'CA103'  # batch 1
subject = 'CA101'   # batch 2
session = '1'
task    = 'dur'
run     = '01'
runs    = ['01', '02', '03', '04', '05']
...
def export_subjects(): # one-time for each batch
    ...
    df = pd.DataFrame({'subject': subjects,
                       'batch': batch})
    f_save = 'subjects_batch_{}.csv'.format(batch)
    df.to_csv(f_save, index=False)
    print('Saved to {}.'.format(f_save))
    return None
...
def max_filter(subject, session, task, runs):
    ...
    for ri, run in enumerate(runs):
        ...
        if False:
            data.compute_psd(fmax=60, n_fft=1000).plot()
            data_sss.compute_psd(fmax=60, n_fft=1000).plot()
...
if False: # example ICA artifact removal
    import matplotlib
    matplotlib.use('TkAgg')  # shift between tk and qt to solve the "fig unable to show" problem
    from code_pyfile.FLUX import *
    subjects = get_subjects()
    subject = subjects[48]  # update the number
    ...
    ics_ex = [1,9,14] # change according to inspection
...
def time_frequency(subject, session, task):
    ...
    for ci, condition in enumerate(conditions):
        ...
        deriv_path = BIDSPath(subject=subject, session=session,
                              ...
                              root=os.path.join(deriv_root, 'Analysis'), extension='.fif', check=False)
        #tf = epochs[condition].compute_tfr(method='multitaper',
        ...
        # output tfr
        f_main = os.path.join('data_tfr', 'batch_{}'.format(batch), 'sub-{}'.format(subject), 'ses-{}'.format(session))
